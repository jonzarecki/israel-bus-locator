...
routes = pd.DataFrame(
    stride.get(
        "/gtfs_routes/list",
        {"route_short_name": 56, "date_from": "2025-01-18", "date_to": "2025-01-18"},
    )
)
...
siri_vehicle_locations_480 = pd.DataFrame(
    stride.iterate(
        "/siri_vehicle_locations/list",
        {
            "siri_routes__line_ref": "7020",
            "siri_rides__schedualed_start_time_from": datetime.datetime(
                2025, 1, 18, tzinfo=tz.gettz("Israel")
            ),
            "siri_rides__schedualed_start_time_to": datetime.datetime(
                2025, 1, 18, tzinfo=tz.gettz("Israel")
            )
