...
def split_by_ride_id(locations_df: pd.DataFrame) -> List[pd.DataFrame]:
    ...
    return [locations_df[locations_df['siri_ride__id'] == ride_id].copy() for ride_id in ride_ids]


def plot_distances_for_rides(ride_dfs: List[pd.DataFrame], ref_point: Tuple[float, float] = (32.090260, 34.782621)):
    ...
    for i, df in enumerate(ride_dfs):
        ...
        sorted_locations['distance_from_ref'] = (
            ((sorted_locations['lat'] - ref_point[0])**2 + 
             (sorted_locations['lon'] - ref_point[1])**2)**0.5
        )
        
        # Plot with a different color and label for each ride
        plt.plot(sorted_locations['recorded_at_time'], 
                sorted_locations['distance_from_ref'], 
                marker='o', 
...
def calculate_distance_to_point(lat: float, lon: float, ref_point: Tuple[float, float]) -> float:
...
def get_current_distances_to_ref(locations_df: pd.DataFrame, ref_point: Tuple[float, float] = (32.090260, 34.782621)) -> Dict[str, Dict]:
    ...
    for df in ride_dfs:
        ...
        current_distances[ride_id] = {
            'current_distance': calculate_distance_to_point(latest['lat'], latest['lon'], ref_point),
...
if __name__ == '__main__':
    ...
    siri_vehicle_locations_480['distance_from_ref'] = (
        ((siri_vehicle_locations_480['lat'] - ref_point[0])**2 + 
         (siri_vehicle_locations_480['lon'] - ref_point[1])**2)**0.5
    )
    ...
    plt.plot(sorted_locations['recorded_at_time'], sorted_locations['distance_from_ref'], marker='o')
